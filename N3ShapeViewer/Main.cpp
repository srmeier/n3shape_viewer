...
int SDL_main(int argc, char* argv[]) {
	...
	debugMode = SDL_TRUE;
	repentMode = (SDL_bool) atoi(argv[1]);

	// NOTE: get the filename for the shape
	char buffer[0xFF];
	//sprintf(buffer, "%s.n3shape", argv[2]);
	sprintf(buffer, "%s", argv[2]);

	FILE* fpShape = fopen(buffer, "rb");

	if(fpShape == NULL) {
		printf("Cannot find the shape file! (%s)\n", buffer);
		system("pause");
		exit(-1);
	}
	/*
	*/
	...
	if(nL0 > 0) {
		char szFN0[NAME_LENGTH] = {};
		memset(&szFN0, 0, (nL0+1));

		fread(&szFN0, sizeof(char), nL0, fpShape);
		ByteFS += sizeof(char)*nL0;

		if(debugMode) {
			printf("\nNOTE: This shape has the name \"%s\"\n", szFN0);
		}
	}
	...
	ByteFS += sizeof(glm::vec3);

	if(debugMode) {
		printf("\nNOTE: Position-> (%f %f %f)\n",
			m_vPos0.x, m_vPos0.y, m_vPos0.z
		);
	}
	...
	ByteFS += sizeof(glm::quat);

	if(debugMode) {
		printf("\nNOTE: Quat-> (%f %f %f %f)\n",
			m_qRot0.x, m_qRot0.y, m_qRot0.z, m_qRot0.w
		);
	}
	...
	if(debugMode) {
		printf("\nNOTE: Scale-> (%f %f %f)\n",
			m_vScale0.x, m_vScale0.y, m_vScale0.z
		);
	}
	...
	*/
	...
	if(!repentMode) {
		...
		if(debugMode) {
			printf("\nNOTE: Len of MeshCollisionFN-> %d\n", nL1);
		}

		// NOTE: if the shape has a collision mesh read it in
		if(nL1 > 0) {
			char* szFN1 = new char[(nL1+1)];
